        ...
        if let sublayers = layer.sublayers {
            for (index,sublayer) in (layer.sublayers as [CALayer]!).enumerated() {
                if let sublayer = sublayer as? CAShapeLayer {
                    ...
                    animation.keyTimes = [0, NSNumber(value: beginTime/animation.duration), NSNumber(value: beginTime/animation.duration), NSNumber(value: (beginTime + Double(animation.duration)/(Double(numberOfWaves) - 2.5))/animation.duration)]
